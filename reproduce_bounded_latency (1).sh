...
# --- build v7ra (seek) from the shipped source ---
echo "=== BUILD v7ra ==="
...
# --- helper: build encoder with a given min_match threshold set (applied FROM base 6/8/10/12) ---
# uses python point-replace (sed is unsafe: 'return 12' matches both dist<128 line and default)
build_encoder () {
  local A=$1 B=$2 C=$3 D=$4 OUT=$5
  cp aceapex_depth.cpp /tmp/_enc.cpp   # base 6/8/10/12 from repo
...
# --- latency scan on TUNED (Paper 5 operating baseline) ---
scan_tuned () {
  local SRC="$1" NAME="$2"
...
# --- depth-cap equalizer: tuned vs aggr on chr1 ---
echo ""
