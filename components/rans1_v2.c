...
static int enc_chunk(const uint8_t *in, int n, uint8_t *out, int *hdr_bytes) {
    ...
    *hp++ = 0; *hp++ = (uint8_t)K;
    ...
    uint8_t *sp = scratch + sizeof scratch;
    uint32_t x[RANS_N];
...
#define EL(k) do {                                                        \
        ...
        uint32_t xx = x[k];                                               \
        int c_ = (xx > e->xmax) * 2;                                      \
        memcpy(sp - 2, &xx, 2);          /* безусловный store, эталон */  \
        xx >>= c_ * 8;                                                    \
        sp -= c_;                                                         \
...
#undef EL
    for (int j = RANS_N - 1; j >= 0; j--) {
        sp -= 4;
        sp[0] = (uint8_t)x[j]; sp[1] = (uint8_t)(x[j] >> 8);
        sp[2] = (uint8_t)(x[j] >> 16); sp[3] = (uint8_t)(x[j] >> 24);
    }

    int payload = (int)(scratch + sizeof scratch - sp);
    int hdr = (int)(hp - out);
    if (hdr + payload + 2 >= n + 1) { out[0] = 1; memcpy(out + 1, in, n); return 1 + n; }
    memcpy(hp, sp, payload);
    hp[payload] = 0; hp[payload + 1] = 0;      /* паддинг для спекулятивного чтения */
    *hdr_bytes = hdr;
...
int dec_core(const uint8_t *p, uint8_t *out, int n, DecTab *dt,
    ...
    uint32_t x[16];
    for (int j = 0; j < N; j++) {
        x[j] = (uint32_t)p[0] | ((uint32_t)p[1] << 8)
