...
__global__ void k_match(uint8_t*out,const uint32_t*tpos,const uint32_t*tsrc,const uint32_t*tlen,const uint32_t*ord,uint32_t base,uint32_t cnt){
    // один WARP на токен: 32 нити делят копирование
    uint32_t gid=blockIdx.x*blockDim.x+threadIdx.x;
    uint32_t i=gid>>5, lane=gid&31;
    if(i>=cnt)return;
    uint32_t ti=ord[base+i],dst=tpos[ti],src=tsrc[ti],len=tlen[ti];
    uint32_t d=dst-src;
    if(d<len){ for(uint32_t k=lane;k<len;k+=32) out[dst+k]=out[src+(k%d)]; }
    else     { for(uint32_t k=lane;k<len;k+=32) out[dst+k]=out[src+k]; }
}
...
int main(int argc,char**argv){
    ...
    for(int L=0;L<=ml;L++) if(cnt_l[L])  // ВСЕ токены — матчи, используем k_match
        k_match<<<(cnt_l[L]*32+255)/256,256,0,s>>>(d_out,d_tp,d_ts,d_tl,d_ord,off[L],cnt_l[L]);
