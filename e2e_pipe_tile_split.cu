...
int main(int argc, char** argv){
    ...
    // ---- TIMED: split into ANS-decode layer and match layer separately ----
    cudaEvent_t ta,tb,tc; CK(cudaEventCreate(&ta)); CK(cudaEventCreate(&tb)); CK(cudaEventCreate(&tc));
    CK(cudaEventRecord(ta));
    tile_decode(TL,res,s2); tile_decode(TO,res,s2); tile_decode(TN,res,s2); tile_decode(TC,res,s2);
    CK(cudaStreamSynchronize(s2));
    CK(cudaEventRecord(tb));   // between ANS-decode and match
    CK(cudaMemcpyAsync(dCTR,&zero,4,cudaMemcpyHostToDevice));
    kern<<<grid,TPB>>>(TL.dFlat,TO.dFlat,TN.dFlat,TC.dFlat,dBO,nb,hdr.orig_size,hdr.block_size,dOUT,dCTR,nb);
    CK(cudaEventRecord(tc)); CK(cudaEventSynchronize(tc));
    CK(cudaGetLastError());
    float ms_ans=0, ms_match=0, ms_total=0;
    CK(cudaEventElapsedTime(&ms_ans,   ta,tb));
    CK(cudaEventElapsedTime(&ms_match, tb,tc));
    CK(cudaEventElapsedTime(&ms_total, ta,tc));
    double gbs = hdr.orig_size/(ms_total*1e-3)/1e9;
    printf("[timed] FULL-PIPE-TILE G=%d: total=%.3f ms -> %.1f GB/s\n", G, ms_total, gbs);
    printf("        SPLIT: ANS-decode=%.3f ms (%.0f%%)  match=%.3f ms (%.0f%%)\n",
        ms_ans, 100.0*ms_ans/ms_total, ms_match, 100.0*ms_match/ms_total);
    printf("        implied: ANS-layer=%.1f GB/s  match-layer=%.1f GB/s (if each ran alone on full output)\n",
        hdr.orig_size/(ms_ans*1e-3)/1e9, hdr.orig_size/(ms_match*1e-3)/1e9);
