...
// prep: ANS-encode stream into compressed buffers (BEFORE timer = "data already compressed")
struct AnsPrep {
  vector<void*> comp_ptrs;      // compressed buffers (input to decode)
  vector<void*> dec_ptrs;       // decode targets (original device locations)
...
static AnsPrep ans_prep(uint8_t* dStream, const vector<BlockOffsets>& bo,
  ...
  P.n=in_ptrs.size(); if(P.n==0){printf("  %s: empty\n",name);return P;}
  ...
  uint64_t comp=0,raw=0; for(uint32_t i=0;i<P.n;i++){comp+=cs[i];raw+=inSizes[i];}
  printf("  %s: %u blocks, raw=%.1fMB comp=%.1fMB ratio=%.2f (prepped, will decode in timer)\n",
         name,P.n,raw/1e6,comp/1e6,(double)raw/comp);
  cudaFree(d_cs);
  return P;
}
// decode: ANS-decompress prepped stream back into place (INSIDE timer)
...
int main(int argc, char** argv){
    ...
    // ===== PREP: ANS-encode 4 streams BEFORE timer (data now "compressed on device") =====
    auto res2 = dietgpu::makeStackMemory((size_t)2*1024*1024*1024);
    cudaStream_t s2; cudaStreamCreate(&s2);
    printf("[PREP] ANS-encoding streams (blocks %u..%u):\n", rstart, rend);
    AnsPrep pL=ans_prep(dLIT,boffs,&BlockOffsets::lit_off,&BlockOffsets::lit_sz,rstart,rend,res2,s2,"LIT");
    AnsPrep pO=ans_prep(dOFF,boffs,&BlockOffsets::off_off,&BlockOffsets::off_sz,rstart,rend,res2,s2,"OFF");
    AnsPrep pN=ans_prep(dLEN,boffs,&BlockOffsets::len_off,&BlockOffsets::len_sz,rstart,rend,res2,s2,"LEN");
    AnsPrep pC=ans_prep(dCMD,boffs,&BlockOffsets::cmd_off,&BlockOffsets::cmd_sz,rstart,rend,res2,s2,"CMD");
    printf("[PREP] done. Timed region will run ANS-decode(4 streams)+match together.\n");
    ...
    ans_decode(pL,res2,s2); ans_decode(pO,res2,s2);
    ans_decode(pN,res2,s2); ans_decode(pC,res2,s2);
    ...
    printf("[timed] FULL-PIPE (ANS+match) G=%d blocks[%u..%u): %.3f ms (%.1f us) -> %.1f GB/s of range\n", G, rstart, rend, ms, ms*1000.0, rbytes/(ms*1e-3)/1e9);
    ...
    ans_prep_free(pL); ans_prep_free(pO); ans_prep_free(pN); ans_prep_free(pC);
