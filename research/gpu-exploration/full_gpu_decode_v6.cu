// =============================================================================
// full_gpu_decode_v6.cu — ACEAPEX decode v6: SUB-WARP groups (G lanes per block)
//
...
int main(int argc, char** argv){
    ...
    void (*kern)(const uint8_t*,const uint8_t*,const uint8_t*,const uint8_t*,const BlockOffsets*,uint32_t,uint64_t,uint32_t,uint8_t*,uint32_t*)
    ...
    uint64_t lanes_needed=(uint64_t)nb*G;
    ...
    kern<<<grid,TPB>>>(dLIT,dOFF,dLEN,dCMD,dBO,nb,hdr.orig_size,hdr.block_size,dOUT,dCTR);
    ...
    printf("[timed] v6 G=%d decode: %.2f ms -> %.1f GB/s\n", G, ms, hdr.orig_size/(ms*1e-3)/1e9);
