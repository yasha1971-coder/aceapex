...
int main(int argc,char**argv){
    ...
    thrust::sequence(d_sa.begin(), d_sa.end());       // 0..n-1
    thrust::sort(d_sa.begin(), d_sa.end(), SuffixLess(dText,n));  // floor SA build
    ...
    // --- 3+4. кандидаты (parallel, absolute offsets) ---
    uint32_t* dBsrc; CK(cudaMalloc(&dBsrc,n*4));
    uint32_t* dBlen; CK(cudaMalloc(&dBlen,n*4));
    uint32_t W = 64;  // окно поиска кандидатов (floor; из sa_v2: w=8 хватает на реальных)
