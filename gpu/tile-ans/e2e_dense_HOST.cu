...
int main(int argc, char** argv){
    ...
    printf("OCCUPANCY block_size=%u nb=%lu maxblk=%d nsm=%d nsm_x_maxblk=%d want=%u grid=%u SATURATED=%s\n", hdr.block_size,(unsigned long)nb,maxblk,nsm,nsm*maxblk,want,grid,(grid>=(uint32_t)(nsm*maxblk))?"YES":"no");

    // D1-dense: dense triplet array indexed by cmd_off (one slot per command byte)
    size_t ntrip = (size_t)hdr.zcmd_sz > 0 ? 0 : 0;
    {   // total CMD bytes = last block's cmd_off + cmd_sz
        BlockOffsets lastbo; CK(cudaMemcpy(&lastbo, dBO+(nb-1), sizeof(BlockOffsets), cudaMemcpyDeviceToHost));
        ntrip = (size_t)lastbo.cmd_off + (size_t)lastbo.cmd_sz + 64;
    }
    D2PTrip* dTRIP=nullptr; uint32_t* dC2=nullptr;
    CK(cudaMalloc(&dTRIP, ntrip * sizeof(D2PTrip)));
    CK(cudaMalloc(&dC2, 4));
    size_t shmem = (size_t)(TPB/32) * 32 * D2P_Q * (sizeof(D2PTrip) + sizeof(uint32_t));
    printf("[D1] TRIP slots=%.1fM = %.0f MB   shmem/blk=%zu B\n",
           ntrip/1e6, ntrip*sizeof(D2PTrip)/1e6, shmem);
    ...
    CK(cudaMemset(dCTR,0,4)); CK(cudaMemset(dC2,0,4));
    CK(cudaMemset(dTRIP,0,ntrip*sizeof(D2PTrip)));
    k_parse_d2p<<<grid,TPB,shmem>>>(TL.dFlat,TO.dFlat,TN.dFlat,TC.dFlat,dBO,nb,hdr.orig_size,hdr.block_size,dTRIP,dCTR);
    k_copy_d2p<<<grid,TPB>>>(TL.dFlat,dTRIP,dBO,nb,hdr.block_size,dOUT,dC2);
    ...
    tile_decode(TL,res,s2); tile_decode(TO,res,s2); tile_decode(TN,res,s2); tile_decode(TC,res,s2);
    CK(cudaStreamSynchronize(s2));
    CK(cudaMemsetAsync(dCTR,0,4)); CK(cudaMemsetAsync(dC2,0,4));
    CK(cudaMemsetAsync(dTRIP,0,ntrip*sizeof(D2PTrip)));
    cudaEvent_t tA; CK(cudaEventCreate(&tA));
    k_parse_d2p<<<grid,TPB,shmem>>>(TL.dFlat,TO.dFlat,TN.dFlat,TC.dFlat,dBO,nb,hdr.orig_size,hdr.block_size,dTRIP,dCTR);
    CK(cudaEventRecord(tA));
    k_copy_d2p<<<grid,TPB>>>(TL.dFlat,dTRIP,dBO,nb,hdr.block_size,dOUT,dC2);
    CK(cudaEventRecord(t1));
    CK(cudaEventSynchronize(t1));
    { float a1=0,b1=0; CK(cudaEventElapsedTime(&a1,t0,tA)); CK(cudaEventElapsedTime(&b1,tA,t1));
      printf("[SPLIT] ANS+parse = %.3f ms  |  copy = %.3f ms\n", a1, b1); } CK(cudaEventSynchronize(t1));
    CK(cudaGetLastError());
