        ...
        for (;;) {
            ...
            if (active) {
                while (n < D2P_Q && out_pos < dst_size && cp < cmd_sz) {
                    ...
                    t.src   = isLit ? (lit_base + aux) : (uint32_t)(bbase + out_pos - aux);
                    T[lane * D2P_Q + n]    = t;
                           ...
                           uint32_t* __restrict__ blk_ctr)
{
    ...
    for (;;) {
        ...
        for (uint32_t k = 0; k < nt; k++) {
            D2PTrip t = run[k];
            if (t.len == 0) continue;                // empty slot (0xFF marker)
            if (t.isLit) {
                for (uint32_t i = lane; i < t.len; i += 32)
                    out[t.dst + i] = LIT[t.src + i];
            } else {
                uint32_t d = t.dst - t.src;
                if (d >= t.len) {
                    for (uint32_t i = lane; i < t.len; i += 32)
