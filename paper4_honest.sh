...
for combo in "fastq:$DATA/NA12878_REAL.fastq:16384" "fastq:$DATA/NA12878_REAL.fastq:32768" "enwik9:$DATA/enwik9:16384" "silesia:$DATA/silesia.tar:16384" "chr1:$DATA/hg38/chr1.fa:32768"; do
...
for bs in 16384 65536 262144; do
  export ACEAPEX_BS=$bs
  r=$(run_tile "$DATA/NA12878_REAL.fastq" $bs)
...
if [ -x ./e2e_pipe_tile_split ]; then
  ...
  $DEPTH c --in "$DATA/NA12878_REAL.fastq" --out /tmp/c.aet --threads 8 2>/dev/null
  $DEPTH d --in /tmp/c.aet --out /tmp/c_dec 2>/dev/null
  ./e2e_pipe_tile_split streams.bin "$DATA/NA12878_REAL.fastq" 2>&1 | grep -E "SPLIT|implied" | tee -a "$OUT"
  unset ACEAPEX_BS; rm -f /tmp/c.aet /tmp/c_dec streams.bin
...
for combo in "fastq:$DATA/NA12878_REAL.fastq:16384" "enwik9:$DATA/enwik9:16384" "silesia:$DATA/silesia.tar:16384" "chr1:$DATA/hg38/chr1.fa:32768"; do
  name=$(echo $combo|cut -d: -f1); src=$(echo $combo|cut -d: -f2); bs=$(echo $combo|cut -d: -f3)
  [ ! -f "$src" ] && continue
  export ACEAPEX_BS=$bs
  $DEPTH c --in "$src" --out /tmp/c.aet --threads 8 2>/dev/null
